...
class FHIR_Facade_Server(Resource):
    ...
    def get(self, resource):
        ...
        #try:
            #prov_conf = params["provision_config"]
        #except:
        ...
        if(resource in RESOURCE_PATHS):
            ...
            response = s.get(SERVER_URL + resource, params=params, headers=request.headers).json()
    ...
    def post(self, resource, search='false'):
        ...
        if(search == '_search'):
            ...
            if (resource in RESOURCE_PATHS):
                #Check if resource has been configured properly
                if(RESOURCE_PATHS[resource]["Date"]=="" or RESOURCE_PATHS[resource]["Subject"]==""):
                    return f"The requested resource has not been configured. Please add the necessary configuration. Or if you dont require consent for the requested resources, use the fhir-server endpoint ({SERVER_URL}) directly."
                    
                ALL_CONSENTS = getAllConsents(SERVER_URL)
                s = requests.session()
                response = s.post(SERVER_URL + resource, data=params, headers=request.headers)
                entries = response.json()["entry"]
                return matchResourcesWithConsents(resources=entries,consents=ALL_CONSENTS,resource_config=RESOURCE_PATHS[resource])
            else:
                return f"The requested resource has not been configured. Please add the necessary configuration. Or if you dont require consent for the requested resources, use the fhir-server endpoint ({SERVER_URL}) directly."
        else:
